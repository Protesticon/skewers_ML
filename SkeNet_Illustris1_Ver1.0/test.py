...
def test_accuracy(test_block_i, test_outp_i, test_ske_i, test_FGPA_i,
    ...
    FGPA4fft = (test_FGPA_i-F_mean[2])/F_mean[2]
    ...
    fft_FGPA = np.absolute(np.fft.fft(FGPA4fft))[:int(ske_len/2)]
    onePS_outp = np.zeros(bins)
    onePS_test = np.zeros(bins)
    onePS_FGPA = np.zeros(bins)
    for jj in range(bins):
        ...
        onePS_FGPA[jj]  = (rvax_t*fft_FGPA**2)[bin_bl[jj]].mean() * 2
    rvaxis = rvaxis * 2 * np.pi / v_end
    onePS_outp = onePS_outp * 2 * np.pi / v_end
    onePS_test = onePS_test * 2 * np.pi / v_end
    onePS_FGPA = onePS_FGPA * 2 * np.pi / v_end
    
    accuracy_i = np.ones(shape=2) * np.nan
    rela_err_i = np.ones(shape=2) * np.nan
    accuracy_i[0] = (np.abs(onePS_outp-onePS_test)/onePS_test).mean()
    accuracy_i[1] = (np.abs(onePS_FGPA-onePS_test)/onePS_test).mean()
    rela_err_i[0] = (np.abs(onePS_outp-onePS_test)/onePS_test).std()
    rela_err_i[1] = (np.abs(onePS_FGPA-onePS_test)/onePS_test).std()
    
    if plot == True:
        fig = plt.figure(figsize=(12,8))
        # plot skewers in F
        axes1 = fig.add_subplot(2,1,1)
        p1, = axes1.plot(vaxis, test_outp_i, label='ML', alpha=0.7)
        p2, = axes1.plot(vaxis, test_ske_i, label='Hydro', alpha=0.5)
        p5, = axes1.plot(vaxis, test_FGPA_i, label='FGPA', alpha=0.5)
        axes1.set_title('y%03dz%03d.png'%(test_block_i[0], test_block_i[1]), fontsize=18)
        ...
        axes2.plot(rvaxis, onePS_outp, label='ML')
        axes2.plot(rvaxis, onePS_test, label='Hydro', alpha=0.5)
        axes2.plot(rvaxis, onePS_FGPA, label='FGPA', alpha=0.5)
        ...
        p3 = axes3.hist(test_outp_i, bins=np.arange(0,1.05,0.05),
                  density=True, histtype='step', label='ML')
        p4 = axes3.hist(test_ske_i, bins=np.arange(0,1.05,0.05),
                  density=True, histtype='step', label='Hydro', alpha=0.5)
        p6 = axes3.hist(test_FGPA_i, bins=np.arange(0,1.05,0.05),
                  density=True, histtype='step', label='FGPA', alpha=0.5)
        ...
        customs = [p1, p2,
                  ...
                  Line2D([0], [0], marker='o', color='w',
                              markerfacecolor='k', markersize=5),
                  p5]
        axes3.legend(customs, [p1.get_label(), p2.get_label(), '$m=%.3f$'%accuracy_i[0],
                            '$s=%.3f$'%rela_err_i[0], p5.get_label(),],
                     fontsize=18, bbox_to_anchor=(1.06,1.5))
    ...
    stat_i = np.array([rvaxis, onePS_outp, onePS_test,
                       onePS_FGPA, accuracy_i, rela_err_i])
    if plot == True:
        return (stat_i, fig)
    if plot == False:
        return stat_i