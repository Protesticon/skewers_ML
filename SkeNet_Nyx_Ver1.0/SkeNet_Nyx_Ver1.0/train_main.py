...
for epoch in range(num_epochs):
    ...
    np.save('params/4Defense/tra_loss_l', tra_loss_l)
    np.save('params/4Defense/val_loss_l', val_loss_l)
    ...
    plt.savefig('params/4Defense/loss_process.png', dpi=500, bbox_inches='tight')

    # remember best prec@1 and save checkpoint if desired
    # is_best = prec1 > best_prec1
    torch.save(model.state_dict(), "params/4Defense/params_%s_%d.pkl"%(time.strftime("%Y-%m-%d_%H:%M:%S", localtime), epoch+1))
