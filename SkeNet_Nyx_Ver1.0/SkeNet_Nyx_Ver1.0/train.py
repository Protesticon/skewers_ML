...
def train(train_ske, train_block, DM_general, DM_param,
    ...
    for i, train_data in enumerate(train_ske, 0):
        ...
        if (i+1) % 100 == 0:
            ...
            tra_loss_l = np.append(tra_loss_l, [(i+1)/train_ske.shape[0]+epoch, loss.item()])
        if ((i+1) % 100 == 0) & (epoch==0): 
            print ("SAVING MODEL!")
            torch.save(model.state_dict(),
                       "params/4Defense/HyPhy_%s_%d_%d"%(time.strftime("%Y-%m-%d_%H:%M:%S", localtime), epoch, i+1))
    
    return losses.avg, tra_loss_l
