...
def train(train_ske, train_block, DM_general, DM_param,
        batch_size, train_size, model, mdn_loss, optimizer,
    ...
    for i, train_data in enumerate(train_ske, 0):
        ...
        if (i+1) % 100 == 0:
            print ("Epoch [{:{}d}/{}], Step [{:{}d}/{}] Loss: {:.4f}, Time: {:.4f}"
                   .format(epoch+1, int(np.log10(num_epochs)+1), num_epochs,
                           i+1, int(np.log10(train_ske.shape[0])+1), train_ske.shape[0], 
